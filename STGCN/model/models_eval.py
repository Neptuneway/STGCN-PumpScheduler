...
class STGCNChebGraphConv(nn.Module):
    ...
    def __init__(self, args, blocks, n_vertex):
        ...
        self.Ko = Ko
        if self.Ko > 1:
            self.output = layers.OutputBlock(Ko, blocks[-3][-1], blocks[-2], blocks[-1][0], n_vertex, args.act_func, args.enable_bias, args.droprate,args.batch_size)
        elif self.Ko == 0:
            ...
            self.fc3 = nn.Linear(n_vertex, 8)  # Output layer for 8 head loss predictions
    def forward(self, x):
        x = self.st_blocks(x)
        if self.Ko > 1:
            x = self.output(x)
            ...
            x = x.view(-1, 1, 1, 8)  # Reshape to batch_size x 1 x 1 x 8
...
class STGCNGraphConv(nn.Module):
    ...
    def forward(self, x):
        x = self.st_blocks(x)#多加一个channel
        if self.Ko > 1:
            x = self.output(x)
        elif self.Ko == 0:
            x = self.fc1(x.permute(0, 2, 3, 1))
