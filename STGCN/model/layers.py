...
class Align(nn.Module):#让c_in大于或等于c_out
...
class ChebGraphConv(nn.Module):
    def __init__(self, c_in, c_out, Ks, gso, bias):##:param Ks: int, kernel size of spatial convolution.param Kt: int, kernel size of temporal convolution
...
class GraphConv(nn.Module):
    def __init__(self, c_in, c_out, gso, bias):#gso邻接矩阵
...
class STConvBlock(nn.Module):
    ...
    def __init__(self, Kt, Ks, n_vertex, last_block_channel, channels, act_func, graph_conv_type, gso, bias, droprate):
        ...
        self.tmp_conv1 = TemporalConvLayer(Kt, last_block_channel, channels[0], n_vertex, act_func)# 时序特征，Kt卷积核大小
        self.graph_conv = GraphConvLayer(graph_conv_type, channels[0], channels[1], Ks, gso, bias)#Ks 切比雪夫算法的阶数
...
class OutputBlock(nn.Module):
    ...
    def __init__(self, Ko, last_block_channel, channels, end_channel, n_vertex, act_func, bias, droprate,batchsize):
        ...
        self.fc2 = nn.Linear(in_features=channels[1], out_features=self.end_channel, bias=bias)
        self.tc1_ln = nn.LayerNorm([n_vertex, channels[0]])
        # The input features to fc3 should be the product of the number of nodes and the number of channels after fc2
        self.fc3=nn.Linear(n_vertex * self.end_channel, self.end_channel)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=droprate)
    ...
    def forward(self, x):
        ...
        # print('\nfc2这是x.shape',x.shape)
        
        x = self.fc3(x.reshape(x.size(0), -1))
        x=x.view(-1, 1, 1, self.end_channel)
        # print('\nfc3展开后这是x.shape',x.shape)
