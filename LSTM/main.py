# -*- coding: UTF-8 -*-
"""
Main program: configuration, data loading, logging, plotting, model training and prediction
"""
...
from data_config import data_file, feature_columns, label_columns

frame = "pytorch"  # Options: "keras", "pytorch", "tensorflow"
if frame == "pytorch":
    from model.model_pytorch import train, predict
else:
    raise Exception("Only PyTorch is supported in this open source version.")
...
class Config:
    ...
    predict_day = 12                        # Number of days to predict

    # Network parameters
    input_size = len(feature_columns)
    output_size = len(label_columns)  # Now 8, matching the new label_columns
    hidden_size = 128                       # LSTM hidden size
    lstm_layers = 4                         # Number of LSTM layers
    dropout_rate = 0.2                      # Dropout rate
    time_step = 12                          # Number of time steps for LSTM

    # Training parameters
    do_train = True
    do_predict = True
    add_train = False                       # Incremental training
    shuffle_train_data = True
    use_cuda = True                         # Use GPU if available
    train_data_rate = 0.95                  # Training data ratio
    valid_data_rate = 0.15                  # Validation data ratio
    batch_size = 288
...
class Data:
    ...
    def get_train_and_valid_data(self):
        ...
        if not self.config.do_continue_train:
            ...
            # Only use the last time step as label
            train_y = [label_data[i+self.config.time_step-1] for i in range(self.train_num-self.config.time_step)]
        else:
            train_x = [feature_data[start_index + i*self.config.time_step : start_index + (i+1)*self.config.time_step]
                       for start_index in range(self.config.time_step)
                       for i in range((self.train_num - start_index) // self.config.time_step)]
            train_y = [label_data[start_index + (i+1)*self.config.time_step - 1]
                       for start_index in range(self.config.time_step)
                       for i in range((self.train_num - start_index) // self.config.time_step)]
