# -*- coding: UTF-8 -*-
"""
PyTorch model implementation for LSTM-based time series prediction.
"""
...
class Net(Module):
    '''
    PyTorch prediction model, including LSTM and Linear output layer.
    Now configured for 8 prediction targets (water head losses).
    '''
    def __init__(self, config):
        super(Net, self).__init__()
        self.lstm = LSTM(input_size=config.input_size, hidden_size=config.hidden_size,
                         num_layers=config.lstm_layers, batch_first=True, dropout=config.dropout_rate)
        self.linear = Linear(in_features=config.hidden_size, out_features=config.output_size)  # output_size=8

    def forward(self, x, hidden=None):
        lstm_out, hidden = self.lstm(x, hidden)
        linear_out = self.linear(lstm_out[:,-1,:]) # Use the last time step output
...
def train(config, logger, train_and_valid_data):
    ...
    for epoch in range(config.epoch):
        ...
        val_loss_lst.append(valid_loss_cur)
        train_loss_lst.append(train_loss_cur)
        logger.info("The train loss is {:.6f}. ".format(train_loss_cur) +
              "The valid loss is {:.6f}.".format(valid_loss_cur)+"lr:{:.10f}".format(optimizer.param_groups[0]['lr']))
        if config.do_train_visualized:
        ...
        if valid_loss_cur < valid_loss_min:
            ...
            if bad_epoch >= config.patience:
                ...
                break
        result_save = pd.DataFrame({'train_loss': train_loss_lst, 'val_loss': val_loss_lst})
    current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M")
    result_save.to_csv(current_time + r'_loss_lstm.csv', float_format='%0.4f')
    return model
